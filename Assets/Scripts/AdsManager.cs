...
//
// UPDATE: the "Ads Mediation" package (com.unity.services.levelplay, 9.5.1)
// is now confirmed installed (Packages/manifest.json + Library/PackageCache).
// The real implementation below was checked line-by-line against that
// package's actual Runtime/Api sources -- LevelPlay.Init/OnInitSuccess/
// OnInitFailed/SetMetaData, LevelPlayRewardedAd/LevelPlayInterstitialAd's
// LoadAd/ShowAd/IsAdReady and event signatures all match as originally
// written. The one real mismatch (LevelPlay.SetConsent is [Obsolete] in this
// version) has been fixed to call LevelPlayPrivacySettings.SetGDPRConsent()
// instead. LEVELPLAY_ENABLED has been added to Project Settings > Player >
// Other Settings > Scripting Define Symbols (Android) so this block is now
// LIVE, not stubbed.
//
// It's still kept behind the LEVELPLAY_ENABLED guard (rather than deleting
// the #if) so the project stays safe to compile even if the package is ever
// removed again -- if that happens, the stub in the #else branch below takes
// over automatically with zero code changes needed elsewhere.
// ============================================================================
...
public class AdsManager : MonoBehaviour
{
    ...
    // LevelPlay.SetConsent(bool) is obsolete as of this package version --
    // LevelPlayPrivacySettings.SetGDPRConsent() is the current API.
